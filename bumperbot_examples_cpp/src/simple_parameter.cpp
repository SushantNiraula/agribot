...
class SimpleParameter : public rclcpp::Node
{
    public:
        SimpleParameter() : Node("simple_parameter")
        {
            ...
            declare_parameter<std::string>("simple_string_param","sushant");
    ...
    private:
        ...
        rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter> & paramters){
            ...
            for(const auto& param : paramters)
            {
                ...
                {
                    RCLCPP_INFO_STREAM(get_logger(), "Param simple_int_param changes! New value is: "<< param.as_int());
                    result.successful = true;
                }

                if(param.get_name() == "simple_string_param" && param.get_type() == rclcpp::ParameterType::PARAMETER_STRING)
                {
                    RCLCPP_INFO_STREAM(get_logger(), "Param simple_string_param changes! New value is: "<< param.as_string());
                    result.successful = true;
                }
            }
